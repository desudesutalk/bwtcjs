...
var bwtc = (function() {
	...
	var Stream = (function() {
		...
		Stream.prototype.seek = function() {
	...
	var Util = (function() {
		...
		Util.compressFileHelper = function(guts) {
			return function(inStream, outStream, props) {
				...
				var suppressFinalByte = tmpOutput[tmpOutput.length-1];
		...
		Util.makeU8Buffer = function(size) {
		...
		return Object.freeze(Util); // ensure constants are recognized as such.
	})();
	...
	var BWT = (function() {
		...
		var SA_IS = function(T, SA, fs, n, k, isbwt) {
			...
			if (k <= 256) {
				...
				if (k <= fs) { B = SA.subarray(n + fs - k);
					flags = 1; } else { B = Util.makeS32Buffer(k);
					flags = 3; }
			} else if (k <= fs) {
				C = SA.subarray(n + fs - k);
				if (k <= (fs - k)) { B = SA.subarray(n + fs - k * 2);
					flags = 0; } else if (k <= 1024) { B = Util.makeS32Buffer(k);
					flags = 2; } else { B = C;
					flags = 8; }
			} else {
	...
	var FenwickModel = (function() {
		...
		FenwickModel.prototype._rescale = function() {
			...
			if (noEscape) { prob = 0; } else if (prob === 0) { prob = (1 << SYM_SHIFT); }
			this.tree[this.numSyms + i] = prob;
	...
	var BWTC = Object.create(null);
	BWTC['enc'] = Util.compressFileHelper(function(input, output, size, props, finalByte) {
	...
	BWTC['dec'] = Util.decompressFileHelper(function(input, output) {
...
if (typeof module != "undefined" && typeof module.exports != "undefined") module.exports = bwtc;
